...
/**
 * HTTP client for calling Python inference service
 * Handles model predictions via REST calls with proper error handling
 */
@Slf4j
@Service
public class InferenceServiceClient {
     ...
     *
     * @param request Iris flower measurements
     * @return Prediction response with class and probabilities
     */
        ...
        try {
            ...
            log.info("Prediction successful: class={}, confidence={}",
                    ...
                    prediction.getProbabilities() != null && !prediction.getProbabilities().isEmpty()
                            ? prediction.getProbabilities().get(prediction.getPredictedClassId())
                            : "N/A");
    ...
    /**
     * Check if inference service is ready to accept requests
     *
